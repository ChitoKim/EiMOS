...
EiMOS_RLC::EiMOS_RLC(ADS1X15 *ext_adc[], float v_unit[], float ref[][4])
    : EiMOS_RLC(&NO_MUX, &DEFAULT_ENV, &DEFAULT_PIN, &DEFAULT_VAL)
{
  memcpy(pin_p->ext_adc, ext_adc, 4 * sizeof(ADS1X15 *));
  memcpy(pin_p->RLC_per_unit, v_unit, 4 * sizeof(float));
...
void
EiMOS_RLC::setWeight(float weight[][4])
{
...
void
EiMOS_RLC::setDebounceCount(unsigned int count)
{
  val_p->debounce_count = count;
}
void
EiMOS_RLC::setSeatButton(int btn)
{
